...
function login_controller($conn) {
    ...
    if (is_post()) {
        ...
        if (is_blank($username) || is_blank($password)) {
            $error = 'Enter both your username/email and password.';
        } else {
            ...
            if (!$user || !password_verify($password, $user['password'])) {
                ...
                log_activity($conn, 'Signed in to ' . role_label($user['role']) . ' portal');
...
/* ============ REGISTER (student / agency / university) ============ */
function register_controller($conn) {
    ...
    $error = '';
    $old = [
        'name' => '', 'email' => '', 'contact' => '', 'username' => '', 'role' => 'student',
        'target_country' => '', 'company_name' => '', 'license_no' => '', 'uni_name' => '', 'country' => '', 'city' => ''
    ];
    ...
    if (is_post()) {
        ...
        $targetCountry = trim($_POST['target_country'] ?? 'United States');
        $companyName   = trim($_POST['company_name'] ?? $name);
        $licenseNo     = trim($_POST['license_no'] ?? '');
        $uniName       = trim($_POST['uni_name'] ?? $name);
        $country       = trim($_POST['country'] ?? '');
        $city          = trim($_POST['city'] ?? '');

        $old = compact('name', 'email', 'contact', 'username', 'role', 'targetCountry', 'companyName', 'licenseNo', 'uniName', 'country', 'city');

        $allowedRoles = ['student', 'agency', 'university'];
        ...
        if (is_blank($name) || is_blank($email) || is_blank($contact)
            ...
            $error = 'Choose a valid account role.';
            ...
            $newUserId = create_user($conn, $name, $email, $contact, $username, $password, $role, 'active');
            if ($newUserId) {
                if ($role === 'agency') {
                    update_agency_profile($conn, $newUserId, $companyName, $licenseNo, '', '', '');
                } elseif ($role === 'university') {
                    update_university_profile($conn, $newUserId, $uniName, $country, $city, '', '', '');
                } elseif ($role === 'student') {
                    update_student_profile($conn, $newUserId, '', $targetCountry, 'Bachelor', '');
                }

                log_activity($conn, 'New ' . $role . ' registered: ' . $username);
